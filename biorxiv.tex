...
\begin{tabular}{|l|l|l|l|l|l|l|}
\hline
% \multicolumn{2}{|l|}{\bf Heading 2}\\ \hline
  {\bf Method} & {\bf Purpose} \\ \hline
...
In contrast to PyBedtools, PyRanges keeps the data in memory, which means that
they do not need to be read into Python datastructures if you want to manipulate
the data in ways possible by bedtools. Furthermore, PyBedtools uses the
temporary disk to store data for the length of the Pybedtools session. In many
instances, this is an onerous requirement, and makes pybedtools less suitable
for writing general purpose libraries intended for use by others. This is
because a library should work without the end user knowing it is there, but when
there is a requirement for external resources such as disk space the end user
needs to make sure they have enough space on their temp drive, or set their temp
drive variable to something else. Also, if the user quits the a running workflow
abruptly with multiple presses of control-c, the tempdrive will fill up with
